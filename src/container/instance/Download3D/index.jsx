...
const Download3D = ({ editorID, experimentID }) => {
  ...
  const result_downloads_3d = [
    { title: "Products (3D SDF/MOL2)", value: `${out_base}/${exp_id}/filtered_3d/overall_filtered_3d.zip` },
  ];
  ...
  return (<>
    ...
    >
      ...
      <div className="flex text-lg text-slate-800 font-semibold relative p-4">
        Ready-to-dock 3D library download
      </div>

      <div id="Download3D_Downloads" className="mx-auto w-full">
        {result_downloads_3d.map(({ title, value }) => (
          <div className="px-2 py-3">{genDownloadLink(title, value)}</div>
        ))}
      </div>
