...
const Linking = () => {
  ...
  const { createImageExport } = useEditor("secondInstance");
  const { createImageExport: createImageExport1 } = useEditor("thirdInstance");
  const {
    growing_smiles,
    selected_Functions,
  ...
  const {
    ...
    structureImage: structureImage1,
  } = state1;
  ...
  const onClickFirst = async (inst) => {
    ...
    const Image = inst === "secondInstance" ? structureImage : structureImage1;
    const createImage =
      inst === "secondInstance" ? createImageExport : createImageExport1;
    ...
    setLoading(true);
    try {
      if (smile && Image.smiles_code !== smile && validFunction(createImage)) {
        await createImage();
      }

      const res = await Api.post("/Callscript_Func", data);
      if (strictValidArrayWithLength(res.data.data)) {
        const value = ConvertRestultFunction(res.data.data, smile);
        dispatch({
